...
def augment_data(images):
    for i in range(0, images.shape[0]):
        ...
        if np.random.random() > 0.75:
            images[i] = random_rotation(images[i], 20, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
            images[i] = random_shear(images[i], 0.2, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
            images[i] = random_shift(images[i], 0.2, 0.2, row_axis=0, col_axis=1, channel_axis=2)
        if np.random.random() > 0.75:
            images[i] = random_zoom(images[i], [0.8,1.2], row_axis=0, col_axis=1, channel_axis=2)
